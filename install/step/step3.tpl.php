...
<script type="text/javascript">
  $(document).ready(function() {
	...
	$("#sso_url").formValidator({onshow:"请输入phpsso地址，必须以'/'结束",onfocus:"请输入phpsso地址，必须以'/'结束",empty:false}).inputValidator({onerror:"地址必须以'/'结束"}).regexValidator({regexp:"http[s]?:\/\/(.+)\/$",onerror:"地址必须以'/'结束"});	
  })
</script>
	<div class="body_box">
        <div class="main_box">
...
<fieldset>
	...
	<div class="content">
    	<input type="radio" name="install_phpsso" id="install_phpsso_1" value="1" checked onclick="set_sso_hidden()">&nbsp;&nbsp;全新安装PHPCMS V9 (已经删除默认即可)<br/>
	...
	<div id="sso_cfg" class="d_n">
		<ul>
			<li>Phpsso地  址:<input type="text" name="sso[sso_url]" id="sso_url" value="https://127.0.0.1/phpsso_server/" class="w260"></li>
			<li>Phpsso用户名:<input type="text" name="sso[username]" value="123"></li>
			<li>Phpsso密  码:<input type="password" name="sso[password]" value="123"></li>
		</ul>
	</div>
</fieldset>
<!--
...
<fieldset>
	<legend>可选数据</legend>
	<div class="content">
    	<label style="width:auto"><input type="checkbox" name="testdata" value="1" >默认测试数据 （用于新手和调试用户）</label>
    </div>
</fieldset>-->
					</form>
					</div>
